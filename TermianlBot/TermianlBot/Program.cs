                        ...
                        case "/on":
                            if (terminalsList.Count != 0)
                            {
                                for (int i = 0; i < terminalsList.Count; i++)
                                {
                                    ...
                                    {
                                        string command1 = $"psexec \\\\{terminalsList[i].ip} -u {terminalsList[i].name} -p Ugh6joosh -i {terminalsList[i].sessionId} -s -d C:/Users/{terminalsList[i].name}/Desktop/Release/Terminal.exe";

                                        string command2 = $"psexec \\\\{terminalsList[i].ip} -u {terminalsList[i].name} -p Ugh6joosh -i {terminalsList[i].sessionId} -s -d taskkill /F /IM explorer.exe";
                        ...
                        case "/off":
                            if (terminalsList.Count != 0)
                            {
                                ...
                                for (int i = 0; i < terminalsList.Count; i++)
                                {
                                    ...
                                    if (terminalsList[i].name == words[1])
                                    {
                                        string command = $"psexec \\\\{terminalsList[i].ip} -i {terminalsList[i].sessionId} -s -d -u {terminalsList[i].name} -p Ugh6joosh taskkill /F /IM Terminal.exe";
                                        string command1 = $"psexec \\\\{terminalsList[i].ip} -i {terminalsList[i].sessionId} -s -d -u {terminalsList[i].name} -p Ugh6joosh C:/Users/{terminalsList[i].name}/Desktop/Release/OpenShow.bat";
        ...
        static void ReceiveMessage()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    String[] words = message.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries); // Преобразуем строку в массив слов (0 - name 1 - ip 2 - sessionId)
                    ...
                    Console.WriteLine($"\nТерминал: {words[0]} sessionId: {words[2]} добавлен!\n");
